...
static TreeNode *wp_gyro_gn_state_ptr = 0;
...
TreeNode * create_w_avs_gn_in_tree(mj_w_avs_gn_t * mj_ptr)
{
static mj_w_avs_gn_t *mj = mj_ptr;
...
TreeNode * create_w_avs_gn_out_tree(mj_w_avs_gn_t * mj_ptr)
{
static mj_w_avs_gn_t * mj = mj_ptr;
TreeNode * node = new TreeNode("OUTPUT",[](){return QVariant();});
node->devIDs = devIDs;
node->devList = devList;
wp_gyro_gn_state_ptr = node->add("wp_gyro_gn_state",[](){return QVariant(mj->out.wp_gyro_gn_state.msg->hdr.cnt);});
wp_gyro_gn_state_ptr->tx_enable = &mj->out.wp_gyro_gn_state.tx_enable;

node->lch()->add("omega",[](){return QVariant(mj->out.wp_gyro_gn_state.msg->omega);})->setData =  [](QVariant data){ mj->out.wp_gyro_gn_state.msg->omega=data.value<float>();return;};
node->lch()->lch()->editable = true;
node->lch()->add("temp",[](){return QVariant(mj->out.wp_gyro_gn_state.msg->temp);})->setData =  [](QVariant data){ mj->out.wp_gyro_gn_state.msg->temp=data.value<float>();return;};
node->lch()->lch()->editable = true;
node->lch()->add("is_valid",[](){return QVariant(mj->out.wp_gyro_gn_state.msg->is_valid);})->setData =  [](QVariant data){ mj->out.wp_gyro_gn_state.msg->is_valid=data.value<bool>();return;};
node->lch()->lch()->editable = true;

return node;
};

TreeNode * create_w_avs_gn_params_tree(mj_w_avs_gn_t * mj_ptr){
static mj_w_avs_gn_t * mj = mj_ptr;
